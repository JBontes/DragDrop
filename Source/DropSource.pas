...
type
  ...
  TAfterDropEvent = procedure(Sender: TObject; DragResult: TDragResult; Optimized: Boolean)
    of object;

  TFeedbackEvent = procedure(Sender: TObject; Effect: LongInt; var UseDefaultCursors: Boolean)
    of object;
  ...
  TDropDataEvent = procedure(Sender: TObject; const FormatEtc: TFormatEtc; out Medium: TStgMedium;
  ...
  // image.
  TGetDragImageEvent = procedure(Sender: TObject; const DragSourceHelper: IDragSourceHelper;
    var Handled: Boolean) of object;
  ...
  /// /////////////////////////////////////////////////////////////////////////////
  TCustomDropSource = class(TDragDropComponent, IDropSource, IDataObject, IAsyncOperation2)
  private
  ...
  protected
    ...
    function GetCanonicalFormatEtc(const FormatEtc: TFormatEtc; out FormatEtcout: TFormatEtc)
      : HRESULT; stdcall;
    function SetData(const FormatEtc: TFormatEtc; var Medium: TStgMedium; fRelease: bool)
      : HRESULT; stdcall;
    function EnumFormatEtc(dwDirection: LongInt; out EnumFormatEtc: IEnumFormatEtc)
      : HRESULT; stdcall;
    ...
    function EndOperation(HRESULT: HRESULT; const pbcReserved: IBindCtx; dwEffects: DWORD)
      : HRESULT; stdcall;
...
var
  ...
  DAD_AutoScroll: function(hWindow: hWnd; var lpSamples: TScrollSamples; var lppt: TPoint)
    : DWORD; stdcall;
  ...
  try
    ...
    try
      ...
      try
        ...
        try
          OleCheck(CoGetInterfaceAndReleaseStream(IStream(FDataObjectStream), IDataObject,
            FDropSource.FAsyncDataObject));
          FDataObjectStream:= nil;
          OleCheck(CoGetInterfaceAndReleaseStream(IStream(FDropSourceStream), IAsyncOperation,
...
function TCustomDropSource.SetData(const FormatEtc: TFormatEtc; var Medium: TStgMedium;
...
function TCustomDropSource.GetData(const FormatEtcIn: TFormatEtc; out Medium: TStgMedium)
  : HRESULT; stdcall;
var
  Handled: Boolean;
begin
...
function TCustomDropSource.GetDataHere(const FormatEtc: TFormatEtc; out Medium: TStgMedium)
  : HRESULT; stdcall;
begin
  Result:= E_NOTIMPL;
end;
...
function TCustomDropSource.EnumFormatEtc(dwDirection: LongInt; out EnumFormatEtc: IEnumFormatEtc)
  : HRESULT; stdcall;
begin
...
function TCustomDropSource.QueryContinueDrag(fEscapePressed: bool; grfKeyState: LongInt)
  : HRESULT; stdcall;
var
...
function TCustomDropSource.EndOperation(HRESULT: HRESULT; const pbcReserved: IBindCtx;
...
begin
  if (AsyncTransfer) then begin
    ...
    if (Assigned(FOnAfterDrop)) then begin
      ...
      FOnAfterDrop(Self, DropResult, (DropResult <> drDropMove) or
...
begin
  ...
  if (FShowImage) then begin
    ...
    if (Succeeded(CoCreateInstance(CLSID_DragDropHelper, nil, CLSCTX_INPROC_SERVER,
      IDragSourceHelper, FDragSourceHelper))) and (FDragSourceHelper <> nil) then begin
      // Create a drag image.
    ...
    if (FDragSourceHelper = nil) and (FImages <> nil) then begin
      IsDraggingImage:= ImageList_BeginDrag(FImages.Handle, FImageIndex, FImageHotSpot.x,
        FImageHotSpot.y);
...
begin
  ...
  if (GenericClipboardFormat.GetDataFromMedium(Self, Medium)) and
    (FRawDataFormat.Assign(GenericClipboardFormat)) then Result:= S_OK;
...
procedure TCustomDropMultiSource.GetCompatibleClipboardFormats(const DataFormatClass
  : TDataFormatClass; ClipboardFormats: TClipboardFormats);
begin
...
begin
  if (ClipboardFormat is TPasteSucceededClipboardFormat) then begin
    ...
    DoOnPaste(DropEffectToDragResult(DropEffect), (DropEffect = DROPEFFECT_MOVE) and
      (PerformedDropEffect = DROPEFFECT_MOVE));
