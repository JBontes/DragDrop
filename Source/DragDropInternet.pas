...
type
  ...
  TURLWClipboardFormat = TUnicodeURLClipboardFormat {$IFDEF VER17_PLUS}deprecated
  {$IFDEF VER20_PLUS}'Use TURLWClipboardFormat instead'{$ENDIF}{$ENDIF};
...
type
  ...
  TMAPIAllocateMore = function(cbSize: ULONG; lpObject: pointer; var lppBuffer: pointer)
    : SCODE; stdcall;
  ...
  TOpenIMsgOnIStg = function(lpMsgSess: pointer; lpAllocateBuffer: pointer; lpAllocateMore: pointer;
    lpFreeBuffer: pointer; lpMalloc: IMalloc; lpMapiSup: pointer; lpStg: IStorage;
    lpfMsgCallRelease: pointer; ulCallerData: ULONG; ulFlags: ULONG; out lppMsg: IUnknown)
    : SCODE; stdcall;
  TOpenIMsgSession = function(lpMalloc: IMalloc; ulFlags: ULONG; var lppMsgSess: pointer)
    : SCODE; stdcall;
    ...
    while (not Result) and (i < URLfile.Count - 1) do begin
      if (CompareText(URLfile[i], sInternetShortcut) = 0) then begin
        ...
        while (not Result) and (i < URLfile.Count) do begin
          ...
          p:= PAnsiChar(s);
          if (StrLIComp(p, 'URL=', Length('URL=')) = 0) then begin
            inc(p, Length('URL='));
            URL:= p;
            Result:= True;
...
function MakeHTML(const s: string): AnsiString;
const
  Header: string = 'Version:0.9' + #13#10 + 'StartHTML:%.00008d' + #13#10 + 'EndHTML:%.00008d' +
    #13#10 + 'StartFragment:%.00008d' + #13#10 + 'EndFragment:%.00008d' + #13#10;
...
const
  Sections: array[boolean, 0 .. 1] of AnsiString = (('StartFragment:', 'EndFragment'),
    ('StartHTML', 'EndHTML'));
var
  n1, n2: integer;
  p: PAnsiChar;
...
begin
  if (CF_INETURLW = 0) then CF_INETURLW:= RegisterClipboardFormat('UniformResourceLocatorW');
  // *** DO NOT LOCALIZE ***
  Result:= CF_INETURLW;
end;
...
type
  TNetscapeImageRec = record
    Size, _Unknown1, Width, Height, HorMargin, VerMargin, Border, OfsLowRes, OfsTitle, OfsURL,
      OfsExtra: DWORD end;
  ...
  var
    ...
    begin
      if (CF_VCARD = 0) then
          CF_VCARD:= RegisterClipboardFormat('+//ISBN 1-887687-00-9::versit::PDI//vCard');
      // *** DO NOT LOCALIZE ***
      Result:= CF_VCARD;
    end;
  ...
  var
    ...
    begin
      if (CF_RFC822 = 0) then
          CF_RFC822:= RegisterClipboardFormat('Internet Message (rfc822/rfc1522)');
      // *** DO NOT LOCALIZE ***
      Result:= CF_RFC822;
    end;
        ...
        *)
        if (Source is TFileClipboardFormat) then begin
          if (FURL = '') then begin
            ...
            if (SameText(ExtractFileExt(s), sInternetShortcutExt)) and (GetURLFromFile(s, NewUrl))
            then begin
              FURL:= NewUrl;
          ...
          if (Source is TFileContentsClipboardFormat) then begin
            ...
            *)
            if (Source is TAnsiFileGroupDescriptorClipboardFormat) then begin
              if (FTitle = '') then begin
                if (TAnsiFileGroupDescriptorClipboardFormat(Source).FileGroupDescriptor^.cItems > 0)
                then begin
                  // Extract the title of an Internet Shortcut
                  s:= string(TAnsiFileGroupDescriptorClipboardFormat(Source)
                    .FileGroupDescriptor^.fgd[0].cFileName);
              ...
              *)
              if (Source is TUnicodeFileGroupDescriptorClipboardFormat) then begin
                if (FTitle = '') then begin
                  if (TUnicodeFileGroupDescriptorClipboardFormat(Source).FileGroupDescriptor^.cItems
                    > 0) then begin
                    // Extract the title of an Internet Shortcut
                    s:= TUnicodeFileGroupDescriptorClipboardFormat(Source).FileGroupDescriptor^.fgd
                      [0].cFileName;
                    if (CompareText(ExtractFileExt(s), sInternetShortcutExt) = 0) then begin
                      FTitle:= ChangeFileExt(s, '');
                      Result:= True;
      ...
      if (Dest is TAnsiTextClipboardFormat) then begin
        ...
        *)
        if (Dest is TFileContentsClipboardFormat) then begin
          TFileContentsClipboardFormat(Dest).Data:= sInternetShortcut + #13#10 + 'URL=' +
            URL + #13#10;
        end
        else
          (*
  ...
  begin
    ...
    if (FMessages[index] = nil) then begin
      ...
      OleCheck(OpenIMsgOnIStg(FSession, pointer(@MAPIAllocateBuffer), pointer(@MAPIAllocateMore),
        pointer(@MAPIFreeBuffer), IMalloc(MAPIGetDefaultMalloc), nil, FStorages[index], nil, 0,
        0, Result));
